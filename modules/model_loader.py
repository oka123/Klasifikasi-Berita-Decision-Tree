...
def load_models():
    ...
    try:
        ...
        ig_selector = joblib.load('ig_selector.pkl')
        dt_model = joblib.load('decision_tree_model.pkl')
        return vectorizer, ig_selector, dt_model
    except Exception as e:
        st.error(f"Error loading models. Ensure '.pkl' files exist in the directory. Details: {e}")
        return None, None, None

def predict_news(text: str, vectorizer, ig_selector, dt_model) -> str:
    """Predict the category of the given news text."""
    if not vectorizer or not ig_selector or not dt_model:
    ...
    text_final = ig_selector.transform(text_tfidf)
